...
exports.login = catchAsync(async (req, res, next) => {
    ...
    if (!email || !password) {
      return next(new AppError('làm ơn nhập email và password!', 400));
    }
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if (!token) {
      return next(
        new AppError('bạn chưa đăng nhập, làm ơn đăng nhập để sài chức năng', 401)
      );
    }
    ...
    if (currentUser.changedPasswordAfter(decoded.iat)) {
      return next(
        new AppError('Người dùng gần đây đã thay đổi mật khẩu! Xin vui lòng đăng nhập lại.', 401)
      );
    }
...
exports.restrictTo = (...roles) => {
    return (req, res, next) => {
      ...
      if (!roles.includes(req.user.role)) {
        return next(
          new AppError('bạn không có thẩm quyền để sài chức năng này', 403)
        );
      }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(new AppError('password của tài khoản bạn không đúng', 401));
  }
